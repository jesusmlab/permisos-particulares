...
<main>
    <div class="row mb-3">
        ...
        <div class="col-lg-4">
            <form name="frmBuscar" action="<? echo BASE_URL; ?>Permisos_c/mantpermisos" method="post">
        ...
        <div class="col-lg-2">
            <?php
            $arrayEstados = ["%%" => "Todas", "S" => "Solicitada", "R" => "Registrada", "C" => "Concedida", "D" => "Denegada", "A" => "Anulada"];
            ?>
            <div class="row">
                <label for="filtro" class="col-sm-4 col-form-label">Estado</label>
                <div class="col-sm-8">
                    <select id="filtro" class="form-control">
                        <?php foreach ($arrayEstados as $clave => $valor) :
                            if ($clave == $filtro) {
                                $sel = "selected";
                            } else {
                                $sel = "";
            ...
            <tbody>
                ...
                if (count($permisos) > 0) {
                    ...
                    foreach ($permisos as $permi) {
                        ...
                        $fpedida = date("d/m/Y", strtotime($permi['fecha_pedida']));
                ?>
                        <tr data-id="<?= $permi['id']; ?>">
                            <td><?= $permi['usuario']; ?></td>
                            <td><?= $permi['apenom']; ?></td>
                            ...
                            <td class="text-center"><?= date("d/m/Y H:i", strtotime($permi['fecha_peticion'])); ?></td>
                            ...
                            <td class="text-center"><?= $permi['fecha_registro'] == "0000-00-00 00:00:00" ? "" : date("d/m/Y H:i", strtotime($permi['fecha_registro'])); ?></td>
